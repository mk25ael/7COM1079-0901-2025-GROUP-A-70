...
# --- 2. Load data -----------------------------------------
# Using read_csv() as requested. Adjust the path to where your CSV is located
# (e.g., if it's in your working directory, just use the filename).
quakes <- read_csv("Japan earthquakes 2001 - 2018.csv")
...
cor.test(quakes_sub$depth, quakes_sub$mag,
         method = "spearman",
         exact = FALSE)

# Original detailed output (kept for reporting)
cor_result <- cor.test(
  quakes_sub$depth,
  quakes_sub$mag,
  method = "spearman",
